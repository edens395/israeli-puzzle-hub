import { SolutionGrid } from '../types/nonogram';
import { calculateColClues, calculateRowClues } from './nonogramUtils';

/**
 * Checks if a given Nonogram solution grid has a unique logical solution without guessing.
 */
export function validateNonogramSolvability(solution: SolutionGrid): { isSolvable: boolean; complexity: number } {
  const height = solution.length;
  const width = solution[0].length;
  const rowClues = calculateRowClues(solution);
  const colClues = calculateColClues(solution);

  // Line solver deduction simulation: 0 = EMPTY, 1 = FILLED, -1 = UNKNOWN
...
function solveLineDeduction(line: number[], clues: number[]): number[] {
  ...
  // Intersection of all valid possibilities
  const result = [...line];
